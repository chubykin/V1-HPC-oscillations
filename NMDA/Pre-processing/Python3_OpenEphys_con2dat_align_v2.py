...
def loadSpikes(filepath):
    ...
    while f.tell() < os.fstat(f.fileno()).st_size:
        ...
        for ch in range(numChannels):
            spikes[currentSpike,:,ch] = (np.float64(wv[ch])-32768)/(gain[currentSpike,ch]/1020)
...
def pack(folderpath, filename='openephys.dat', dref=None,
    chunk_size=10000, start_record=None, stop_record=None, verbose=True,
    **kwargs):
    ...
    for chunk_start in range(start_record, stop_record, chunk_size):
        ...
        # Load the chunk
        data_array, data_aln = loadFolderToArray(folderpath, dtype=np.int16,
        ...
        if len(data_array) == 0:
            break
        data_aln = data_aln.T
        data_aln = data_aln.astype(dtype=np.int16)
...
def _get_sorted_channels(folderpath, recording=None):
    ...
    if recording is None:
        return sorted([int(f.split('_CH')[1].split('.')[0]) for f in os.listdir(folderpath) 
                    if '.continuous' in f and '100_CH' in f]) 
    else:
